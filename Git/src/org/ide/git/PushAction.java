...
/**
 * Acción que, teniendo el repositorio remoto configurado, manda el comando "git
 * push" por consola.
 *
    ...
    private void pushGitAction(Project selectedProject) {
        if (selectedProject != null) {
            ...
            try {
                ...
                if (exitCode == 0) {
                    System.out.println("[GIT] El comando PUSH se ha ejecutado con éxito");
                } else {
                    AuxGit.printProcessErrors(p);
                    System.out.println("[GIT] El comando PUSH ha tenido un error en su ejecución, mire el mensaje de consola");
                }
                p.waitFor();
